...
%X = [linspace(0,3*pi/4,N)',ones(N,1)];%[N,M+1]
%Y = sin(linspace(0,3*pi/4,N)');
X = [linspace(-2*pi,2*pi,N)',ones(N,1)];%[N,M+1]
Y = sin(linspace(-2*pi,2*pi,N)');% + 3*cos(linspace(0,2*pi,N)');
%x = pi + (3*pi-0).*rand(N,M);
%X = [x, ones(N,1)];
%Y = sin(x);
...
%     end
% end
X = [linspace(-pi,3*pi,N)',ones(N,1)];%[N,M+1]
Y = sin(linspace(-pi,3*pi,N)');% + 3*cos(linspace(0,2*pi,N)');
o1 = [tanh(X*W1),ones(N,1)];
o2 = [tanh(o1*W2),ones(N,1)];
y_hat = tanh(o2*W3);
plot(linspace(-pi,3*pi,N)',y_hat,'--');
hold on;
plot(linspace(-pi,3*pi,N)',Y,'r');
